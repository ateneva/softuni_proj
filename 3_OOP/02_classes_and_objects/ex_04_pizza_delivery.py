...
class PizzaDelivery:
    ...
    def remove_ingredient(self, ingredient: str, quantity: int, price_per_quantity: float):
        if self.ordered is False:
            ...
            if ingredient not in self.ingredients:
                ...
                # print(f'{ingredient}: {current_quantity}')
    ...
    def make_order(self):
        self.ordered = True
        print_format = ', '.join([f'{k}: {v}' for k, v in self.ingredients.items()])
        return f"You've ordered pizza {self.name} prepared with {print_format} and the price will be {self.price}lv."
